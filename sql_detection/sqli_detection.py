...
def G_means(token_seq, c_name):
	try:
		g_scores = [tc_dataframe[token][c_name] for token in token_seq]
	except KeyError:
		return 0
...
df = pd.read_csv("plain.csv",names=['raw_sql'])
df['type'] = 'plain'
df_list.append(df)

new_df = pd.read_csv('sqli.csv',names=['raw_sql'])
new_df['type'] = 'sqli'
...
X = dataframe[['token_length', 'entropy','sqli_g_means','plain_g_means']]
...
print("Gradient Boosting Tree Acurracy: " + str(clf.score(X_test, y_test) * 100) + " %. ")
...
def check_statement(statement, note = ''):
	result = classify_statement(statement)
	print("Checking statement : " + str(statement) + " " + note)
	if result == 1:
		print("This is an SQL-Injection.")
	else:
		print("This is a plain-text.")

# check_statement("hero sleep or sit in an unlikely place")
# check_statement("""’Hello UNION Duy AND Thoai""")
# check_statement('Duy UNION ALL SELECT NULL')
check_statement('John Doe', "(normal statement from W3school)")
check_statement("%'--; SELECT DATABASE();")
check_statement("Duy SELECT Thoai")
check_statement("'; SHOW Table;")
check_statement("'SELECT *;")
check_statement("' OR 1 = 1; /*")
