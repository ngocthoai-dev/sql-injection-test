...
router.post('/', sessionChecker, function(req, res) {
  ...
  // console.log(username, password);
    ...
    let query = "SELECT `username`, `password` FROM `accounts` WHERE `username`='" + username + "' AND `role`='admin';";
      ...
      if(err)
        res.send({ msg: "error on server or your invalid input", tagMsg: "username", success:false });
      else {
    		user = user[0];
          ...
          if(password.localeCompare(repassword)){
            ...
            let query = "INSERT INTO `accounts` (`username`, `password`, `role`) VALUES ('" + username + "', '" + hashing.hash(password, {salt: username, rounds: 20}) + "', 'client');";
    				db(query, (err, result)=>{
              if(err)
                res.send({ msg: "error on server or your invalid input", tagMsg: "username", success:false });
              else {
